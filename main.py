...
path_to_current = "./save_env.pkl"
question_mark_pic = os.environ["QUESTION_MARK"]
brahim_id = os.environ["RAHIM_ID"]
game_log_channel_id = int(os.environ["CHANNEL_ID"])
roll_me_daddy_id = int(os.environ["SERVER_ID"])
...
def battle_summary(caster, spellname, target):
    ...
    #Use spell
    battle_recap, dice_roll, modifier = env.use_spell(caster, spellname,
                                                      target)
    ...
    if (caster.state == "stunned"):
        if (caster.state_duration == 0):
            ...
            embed.add_field(
                name="You are **STUNNED**, you can't play this turn.")
            return embed
    # Battle recap
    if (modifier != 0):
...
async def on_message(message):
    ...
    def fight_summary(caster, target):
        msg = caster.name + " " + str(caster.current_hp) + "/" + str(
            caster.max_hp) + "   \*\*\*\*\*   " + target.name + " " + str(
                target.current_hp) + "/" + str(target.max_hp) + "\n"
        return msg
    ...
    if (msg.startswith("!set name ")):
        ...
        set_entity_current_hp(current_hp,
                              env.dict_characters[message.author.id])
        ...
        set_entity_description(description,
                               env.dict_characters[message.author.id])
    ...
    # Set monster parameters
        ...
        if (msg.startswith("set race ")):
            ...
            description = msg.split("set description ", 1)[1]
            set_entity_description(description,
                                   env.dict_monsters[monster_name])
        elif (msg.startswith("set picture ")):
            picture_url = msg.split("set picture ", 1)[1]
    ...
    # pvp spellname target
        ...
        msg_out = env.use_spell(caster=caster,
                                target=target,
                                spellname=spellname)
        ...
        env.dict_battle = {}
        await game_log_channel.send(
            f"Battle Cleared ! requested by <@{message.author.id}>")
    elif (msg.startswith("!battle perception ")):
        battle_monster_name = msg.split("!battle perception ", 1)[1]
        embed = get_monster_perception_profile_embed(battle_monster_name,
                                                     message.author.id)
        await game_log_channel.send(embed=embed)
